...
import { getThemeConfigurationsByCategory, getThemeConfiguration } from '../config/themeConfigs';
...
export class ThemeSelectionService {
  ...
  selectTheme(wizardData: any): ThemeSelectionResult {
    ...
    if (categoryThemes.length > 0) {
      ...
      const scoredThemes = categoryThemes.map(theme => ({
        ...theme,
      ...
      return {
        ...
        alternativeThemes: scoredThemes.slice(1, 4).map(theme => ({
          id: theme.id,
          name: theme.name,
    ...
    // Fallback to general business themes or default
  ...
  private getDefaultThemeSelection(websiteType: string): ThemeSelectionResult {
    // Check if health-wellness-theme is available as fallback for healthcare
    ...
    // Ultimate fallback - use a standard Hugo theme
    return {
      selectedTheme: {
        id: 'ananke',
        name: 'ananke',
      ...
      reason: 'Default Hugo theme selected as fallback',
      alternativeThemes: []
    };
  }
  ...
  validateThemeForWizardData(themeId: string, wizardData: any): {
    ...
    themeConfig.requiredParameters.forEach(param => {
      switch (param) {
        case 'phone':
